...
impl BuiltinAction {
    pub(crate) fn command_name(&self) -> &'static str {
        match self {
            ...
            BuiltinAction::WorkflowManage { .. } | BuiltinAction::WorkflowLaunch { .. } => {
                "workflow"
            }
        }
    }
    ...
    pub(crate) fn args_provided(&self) -> bool {
        match self {
            ...
            BuiltinAction::DeepResearch { .. } | BuiltinAction::WorkflowManage { .. } => true,
...
mod tests {
    ...
    fn connect_is_advertised_before_accounts_or_session_capabilities_exist() {
        let commands = available_commands(&[], CommandAvailability::default(), &[]);
