...
/// Standard workflow/task depth gate. Bandicot's ultra mode additionally uses
/// `enforce_depth_one_tool_restriction` to apply the same pruning earlier.
pub(super) fn strip_task_tools_at_max_depth(
    config: &mut xai_grok_tools::registry::types::ToolServerConfig,
    child_depth: u32,
) -> bool {
    enforce_depth_one_tool_restriction(config, false, child_depth)
}
...
pub(crate) async fn handle_subagent_request(
    ...
    if enforce_depth_one_tool_restriction(
        ...
        tracing::info!(
            subagent_id = %request.id,
            child_depth,
            ultra_depth_one = ctx.ultra_depth_one,
    ...
    xai_grok_telemetry::unified_log::info(
        ...
        Some(
            serde_json::json!(
                ...
                effective_sampling_config.base_url, "has_credential" : has_credential(&
                ...
                .model_id.0.as_ref(), "parent_has_credential" : has_credential(& ctx
