...
impl ModelEntry {
    ...
    /// The provider governing this model's bearer. Static credentials take precedence.
    ...
    let (api_key, base_url, auth_type) = if let Some(key) = model.own_credential() {
        ...
        if crate::util::is_first_party_xai_url(&url) {
            ...
            tracing::warn!(
                model = %info.model,
                base_url = %url,
                "ignoring the xAI API key for a non-xAI endpoint",
            );
            (
                None,
                info.base_url.clone(),
...
/// Resolve auth facts and the credential-provider reference from the same
/// effective model lookup. Keeping these together prevents a provider change
/// from being observed halfway through a session refresh.
pub fn resolve_model_auth_facts_and_provider(
    model_id: &str,
) -> (ModelAuthFacts, Option<crate::auth::AuthProviderRef>) {
    if model_id.is_empty() {
        return (resolve_model_auth_facts(model_id), None);
    }
