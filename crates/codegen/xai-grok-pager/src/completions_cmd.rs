//! `bandicot completions <shell>` — generate shell completion scripts.
//!
...
pub fn run(shell: Shell) {
    let mut cmd = PagerArgs::command().name("bandicot");
    if shell != Shell::Zsh {
        generate(shell, &mut cmd, "bandicot", &mut std::io::stdout());
        return;
    }
    // zsh needs post-processing (see fix_zsh_root_prompt_positional).
    let mut buf = Vec::new();
    generate(shell, &mut cmd, "bandicot", &mut buf);
...
/// `$line[2]` empty, and the dispatch falls through — so `bandicot worktree <TAB>`
...
fn fix_zsh_root_prompt_positional(script: &str) -> String {
    ...
    for (from, to) in [
        ...
        (
            r#"curcontext="${curcontext%:*:*}:bandicot-command-$line[2]:""#,
            r#"curcontext="${curcontext%:*:*}:bandicot-command-$line[1]:""#,
        ),
...
mod tests {
    ...
    fn zsh_script() -> String {
        let mut cmd = PagerArgs::command().name("bandicot");
        let mut buf = Vec::new();
        generate(Shell::Zsh, &mut cmd, "bandicot", &mut buf);
    ...
    // so `bandicot worktree <TAB>` re-offered every top-level command (upstream
    // clap-rs/clap#6282).
    #[test]
